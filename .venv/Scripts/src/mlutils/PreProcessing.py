...
class PreProcessTrainingData():
    ...
    def pre_process_categorical_numeric_columns(self,drop_columns,categorical_columns,numeric_columns):
        self.data_fine_tune = self.data.drop(drop_columns, axis = 1)
        self.data_fine_tune['Churn']=self.data_fine_tune['Churn'].map({"Yes":1,"No":0})
        ...
        self.data_fine_tune_encoded.head()
        self.data_fine_tune_encoded.to_csv(r'../../data/preprocessed/Telco-Customer-Churn-encoded.csv',index=False)
